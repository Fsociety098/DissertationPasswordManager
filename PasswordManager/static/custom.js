...
(function ($) {
    ...
    const options = {
            ...
            errorMessages: {
                ...
                bannedpassword: "This password has been involved in data breaches and is in the top 100k passwords"
            },
            scores: [17, 26, 40, 50],
            verdicts: ["Weak", "Medium", "Strong", "Very Strong"],
            showVerdicts: true,
            raisePower: 1.2,
        ...
        setProgressBar = function ($el, score) {
            ...
            if (score < options.scores[0]) {
                progressbar.addClass("progress-bar").removeClass("progress-bar");
                progressbar.find(".bar").css("width", "10%").css("background", "red");
                if (options.showVerdicts) {
                    $verdict.text(options.verdicts[0]);
                }
            } else if (score >= options.scores[1] && score < options.scores[2]) {
                progressbar.addClass("progress-bar").removeClass("progress-bar");
                progressbar.find(".bar").css("width", "50%").css("background", "orange");
                if (options.showVerdicts) {
                    $verdict.text(options.verdicts[1]);
                }
            } else if (score >= options.scores[2] && score < options.scores[3]) {
                progressbar.addClass("progress-bar").removeClass("progress-bar");
                progressbar.find(".bar").css("width", "75%").css("background", "lightgreen");
                if (options.showVerdicts) {
                    $verdict.text(options.verdicts[2]);
                }
            } else if (score >= options.scores[3]) {
                progressbar.addClass("progress-bar").removeClass("progress-bar");
                progressbar.find(".bar").css("width", "100%").css("background", "green");
                if (options.showVerdicts) {
